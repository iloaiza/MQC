...
function pot_general_layered(R,beta=0.015,offset_delta=0.0001,alpha=0.004,gamma=0.02,delta=0.01,Omega_layered=0.004/sqrt(2),thickness_layered=9,dist=0.16/2,)

    offset=offset_delta.*kron(ones(thickness_layered,1),0:thickness_layered)
    pos=dist.*(-4:1:4)

    D=kron(pos,ones(thickness_layered+1,1))+offset
    ALPHA_MAT=zeros(thickness_layered*(thickness_layered+1),thickness_layered*(thickness_layered+1))
    for i in 2:9*(thickness_layered+1)
        ALPHA_MAT[i,i-1]=alpha
        ALPHA_MAT[i-1,i]=alpha
    end
    ...
    Dreal_layered=D./(mass*Omega_layered^2)
    A_layered=ALPHA_MAT
    ...
    function Vtot(R,beta,gamma,delta)
        Bmat=beta.*eye(thickness_layered+1)
        Cmat=gamma.*eye(thickness_layered+1)
        Dmat=delta.*eye(thickness_layered+1)
    ...
    function dVd(R,D1)
        return mass*Omega_layered^2*(R-D1)
    end
